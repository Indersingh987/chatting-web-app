...
export const cancle = (id,index) => dispatch => {
    ...
    axios.post('/api/request/cancle',{ id },{headers:{'Authorization':`Bearer ${JSON.parse(sessionStorage.getItem('token'))}`}})
    .then(res=>{
...
export const request = (id,index) => dispatch => {
    ...
    axios.post('/api/request/send',{ id },{headers:{'Authorization':`Bearer ${JSON.parse(sessionStorage.getItem('token'))}`}})
    .then(res=>{
