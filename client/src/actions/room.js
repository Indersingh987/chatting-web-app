...
export const getList = (id) => dispatch => {
    ...
    api.post('/api/message',{ id },{headers:{'Authorization':`Bearer ${JSON.parse(sessionStorage.getItem('token'))}`}})
...
export const create = (id,text,time) => dispatch => {
    api.post('/api/message/new',{ id,text,time },{headers:{'Authorization':`Bearer ${JSON.parse(sessionStorage.getItem('token'))}`}})
