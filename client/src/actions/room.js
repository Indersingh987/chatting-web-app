...
export const getList = (id) => dispatch => {
    ...
    axios.post('/api/message',{ id },{headers:{'Authorization':`Bearer ${JSON.parse(sessionStorage.getItem('token'))}`,'Content-Type':'application/x-www-form-urlencoded'}})
...
export const create = (id,text,time) => dispatch => {
    axios.post('/api/message/new',{ id,text,time },{headers:{'Authorization':`Bearer ${JSON.parse(sessionStorage.getItem('token'))}`,'Content-Type':'application/x-www-form-urlencoded'}})
